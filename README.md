...
管理员也可以进入“管理后台 - 系统设置 - 数据备份”，点击“一键备份”立即生成一份备份。

恢复备份时，管理员可以进入“管理后台 - 系统设置 - 数据备份”，点击“查看备份”，选择某个备份并点击“恢复”。系统会先自动创建一份当前数据的 `pre-restore` 安全备份，再恢复到选中的备份。
...
命令行恢复时，建议先停止服务并保留当前 `data/` 副本，再从同一个备份目录恢复 `runtime.sqlite` 和 `papers.json`。
